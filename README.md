...
|--------------------|-----------------------------------------------|:-------------:|:---------------------:|
| `--verbose`        | verbose output (DEPRECATED FLAG)              |    `false`    |        *none*         |
| `--debug`          | debug output (DEPRECATED FLAG)                |    `false`    |        *none*         |
| `--log-json`       | logs in JSON format (DEPRECATED FLAG)         |    `false`    |        *none*         |
...
| Name                       | Description                      |    Default value    | Environment variables |
|----------------------------|----------------------------------|:-------------------:|:---------------------:|
| `--index` (`-i`)           | generate index page              |       `false`       |        *none*         |
| `--disable-l10n`           | disable error pages localization |       `false`       |        *none*         |
| `--config-file="…"` (`-c`) | path to the config file (yaml)   | `./error-pages.yml` |     `CONFIG_FILE`     |

### `serve` command (aliases: `s`, `server`)
...
| `--template-name="…"` (`-t`) | template name (set "random" to use a randomized or "i-said-random" to use a randomized template on each request or "random-daily/random-hourly" daily/hourly randomized) |                     |    `TEMPLATE_NAME`    |
| `--default-error-page="…"`   | default error page                                                                                                                                                       |        `404`        |  `DEFAULT_ERROR_PAGE` |
| `--default-http-code="…"`    | default HTTP response code                                                                                                                                               |        `404`        |  `DEFAULT_HTTP_CODE`  |
| `--show-details`             | show request details in response                                                                                                                                         |       `false`       |    `SHOW_DETAILS`     |
| `--proxy-headers="…"`        | proxy HTTP request headers list (comma-separated)                                                                                                                        |                     |  `PROXY_HTTP_HEADERS` |
| `--disable-l10n`             | disable error pages localization                                                                                                                                         |       `false`       |    `DISABLE_L10N`     |
| `--catch-all`                | catch all pages                                                                                                                                                          |       `false`       |      `CATCH_ALL`      |
